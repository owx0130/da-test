...
file = pd.read_csv("tidal_data/2022_tidal_data.csv", header=None, usecols=[1,2,4], na_filter=False)
tide_2022 = file.to_numpy()
print(tide_2022)

# retrive 2023 tide data from csv file
# change usecols parameter if required. retrieved data should be in DD:"HH:MM":{"H", "L"} format
file = pd.read_csv("tidal_data/2023_tidal_data.csv", header=None, usecols=[1,2,4], na_filter=False)
tide_2023 = file.to_numpy()
print(tide_2023)

# retrive 2024 tide data from csv file
# change usecols parameter if required. retrieved data should be in DD:"HH:MM":{"H", "L"} format
file = pd.read_csv("tidal_data/2024_tidal_data.csv", header=None, usecols=[1,2,4], na_filter=False)
...
for i in range(len(vessel_arrival)):
  ...
  while(day_idx < len(truncated) and time > truncated[day_idx, 1]):
    day_idx += 1

  if (day_idx == len(truncated) or truncated[day_idx, 1] == "Time"):
    tide = truncated[day_idx - 1, 2]
  else:
    tide = "H" if truncated[day_idx, 2] == "L" else "L"
